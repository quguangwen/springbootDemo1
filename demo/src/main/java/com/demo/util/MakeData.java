...
public class MakeData {
    ...
    public static List<NameData> makeData(int dataLength, String dateBirth) {
        // 写一个百家姓的数组
        ...
        for (int i = 0; i < dataLength; i++) {
            ...
            nameData.setDateStr(dateBirth);
            // 再给每个生成的名字加个随机身份证号
            String idCardNumber = makeidCardNumber(nameData.getDateStr());
     ...
     * @return
     */
    private static String makeidCardNumber(String dateBirth) {
    ...
    public static void main(String[] args) {
        ...
        int dataLength = 10;
        String birthday = "19871111";
        LinkedHashMap<String,String>hashMap = new LinkedHashMap();
        List<NameData> dataList = makeData(dataLength,birthday);
        for(int i = 0 ; i< dataList.size();i++){
            SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
            String time = df.format(new Date());
            System.out.println("姓名：" + dataList.get(i).getName() +",身份证号：" + dataList.get(i).getIdCardNumber() + ", 时间：" + time);
            String name = dataList.get(i).getName();
            String mobile = AesUtils.encrypt("18618439839");
            String idcard = AesUtils.encrypt(dataList.get(i).getIdCardNumber());

            String sql = "INSERT INTO lend_app.app_channel_customer"+
                    ...
                    "VALUES " +
                    "("+ i + ",'"+ time +"',1,'" + name +"','" +idcard + "', '"+ mobile +"', 'SALARY', 'SALARY2.0_WISDOM', '024720', '110100', 'baichuan', 'baichuan', null, null, null)";


            try {
                PreparedStatement pstmt = DBConn.getDB().prepareStatement(sql);
                pstmt.execute();
